...
//
// The InboxSDK App ID is read from chrome.storage.sync (set via the
// extension's Options page).  Nothing initialises until the ID is available.
  ...
  chrome.storage.sync.get(['inboxSdkAppId'], ({ inboxSdkAppId }) => {
    if (!inboxSdkAppId) {
      console.warn(
        ...
        'Right-click the extension icon → Options to set one.'
      );
      return;
    }
    ...
    app.ports.insertQuote.subscribe(text => {
      ...
      // No trailing <br>: <blockquote> is block-level so the cursor lands
      // on the next line naturally without an extra blank line.
      const quoteHTML =
        `<blockquote style="${quoteStyle}">${escapeHtml(text)}</blockquote>`;
